...
function initAnimation() {
...

  {
    ...
    window.prevCanvasEl = prevCanvasEl;
  }

  if (prevCanvasEl) {
    // Check if canvasElement actually changed width. If not, do not initialize
    // animation.
    if (
      targetEl.clientWidth - 50 < prevCanvasEl.width &&
      prevCanvasEl.width < targetEl.clientWidth + 50
    ) {
      return;
    }
    gGrid = null;
    targetEl.removeChild(prevCanvasEl);
    // If the frame is already frozen, give it a little more time.
    if (gTick >= TICK_MAX) gTick = gTick - FPS * 2;
  }
